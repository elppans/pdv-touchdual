...
RESOLUCAO_OPERADOR="1024x768"					# Resolução suportada pelo monitor
...
POSICAO_OPERADOR="0x0" # "${LARGURA_OPERADOR}x0"
...
echo -e "
	xrandr --auto --output "$XOPERADOR" --mode "$RESOLUCAO_OPERADOR" --pos "$POSICAO_CLIENTE" --rate "$RATE_OPERADOR" \
...
main() {
	adicionar_resolucao "$XOPERADOR" 1366 768 60           # Exemplo: criar resolução 1024x768 a 60Hz na saída HDMI-2 (adicionar_resolucao "HDMI-2" 1024 768 60)
