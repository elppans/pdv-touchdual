...
for /f "tokens=1,* delims=:" %%a in ('tasklist /v /fi "imagename eq chrome.exe" /fo list ^| findstr "Cliente - Google Chrome"') do (
    for /f "tokens=*" %%i in ("%%b") do set INFACEWEB=%%i
)
echo Encontrado Interface: %INFACEWEB%
C:\opt\webadmin\extra\zpoja.exe "%INFACEWEB%" "2"
